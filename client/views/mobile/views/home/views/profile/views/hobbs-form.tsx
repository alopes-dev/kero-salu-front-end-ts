...
import { postHobbes } from '@services/hobbes'
...
type HobbesFormProps = {
  setter?(): void
}

const HobbesForm: React.FC<HobbesFormProps> = ({ setter }) => {
  const { user } = useContext(AuthContext)
  const { register, handleSubmit, getValues } = useForm()
  const isMounted = useIsMounted()
  const { setLoading } = useAsyncState()

  const handleSave = async ({ designation }: IHobbesAttributes) => {
    setLoading(true)
    try {
      const { error, message, data: res } = await postHobbes({
        designation,
