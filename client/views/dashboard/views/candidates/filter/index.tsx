...
const FilterForm: FC<FilterFormProps> = ({ data, triggerEvent }) => {
  ...
  useEffect(() => {
    if (!value) {
      if (isMounted.current) triggerEvent(candidateParser(data))
      return
    }
    const dataFiltered = data.filter(
      item =>
        `${item.firstName} ${item.lastName}`.toLowerCase().includes(value) ||
        item.birthDate?.toLowerCase().includes(value) ||
        item.createdAt?.toLowerCase().includes(value)
    )
    if (isMounted.current) triggerEvent(candidateParser(dataFiltered))
  }, [debouncedValue])
